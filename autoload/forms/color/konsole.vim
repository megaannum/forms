...
function! forms#color#konsole#ConvertRGB_2_Int256(rn, gn, bn)
"let start = reltime()
  let rn = a:rn
  let gn = a:gn
  let bn = a:bn
...
" ------------------------------------------------------------ 
function! forms#color#konsole#ConvertInt256_2_RGB(nr)
  if (type(a:nr) == g:self#NUMBER_TYPE)
    return s:Int256_2_RGB[a:nr]
  elseif (type(a:nr) == g:self#STRING_TYPE)
    return s:Int256_2_RGB[a:nr]
  else
    throw "forms#color#konsole#ConvertInt256_2_RGB: Bad number: " . string(a:nsstr)
  endif
endfunction
...
call forms#log("Konsole: cube=". string(cube))
...
for r in cube
  for g in cube
    for b in cube
      ...
      call forms#log("    \\ '". (cnt+16) . "': '" . rgbtxt . "',")
      let cnt += 1
    endfor
 endfor
endfor

function! forms#color#konsole#DoIT()
endfunction
endif
